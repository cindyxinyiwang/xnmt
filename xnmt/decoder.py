...
class TreeHierDecoder(RnnDecoder, Serializable):
  ...
  def __init__(self, yaml_context, vocab_size, word_vocab_size, layers=1, input_dim=None, lstm_dim=None,
    ...
    # ruleRNN output + ruleRNN update
    #word_lstm_input += lstm_dim*2
    ...
    if not self.decoding:
      ...
      if is_terminal[0] == 0:
        ...
        if eos:
          ...
          #inp = dy.concatenate([inp, paren_tm1_state, dy.zeros(self.lstm_dim)])
          inp = dy.concatenate([inp, paren_tm1_state])
      ...
      if open_nonterms[-1].label == u'*':
        if trg == Vocab.ES:
          #inp = prev_word_emb
          #prev_word_emb = word_embedder.embed(trg)
          pass
        else:
  ...
  def get_scores(self, tree_dec_state, trg_rule_vocab, is_terminal, label_idx=-1, trg=None):
    """Get scores given a current state.
    ...
    return self.rule_vocab_projector(h_t) + dy.inputTensor(valid_y_mask), len(valid_y_index)
