...
class TrainingStrategy(Serializable):
  ...
  def __call__(self, translator, dec_state, src, trg, pick_src_elem=-1, trg_rule_vocab=None, word_vocab=None):
      return self.loss_calculator(translator, dec_state, src, trg, pick_src_elem, trg_rule_vocab, word_vocab)
  ...
  def __call__(self, translator, dec_state, src, trg, pick_src_elem=-1, trg_rule_vocab=None, word_vocab=None):
    ...
    for i in range(seq_len):
      ...
      if trg_is_list:
          ...
          out = translator.decoder.calc_loss(dec_state, ref_word, trg_rule_vocab)
          if len(out) > 1:
            word_loss, is_terminal = out
          else:
            word_loss = out
            is_terminal = True
      else:
      ...
      if is_terminal:
        terminal_losses.append(word_loss)
      if i < seq_len-1:
        if trg_is_list:
    ...
    return dy.esum(losses), dy.esum(terminal_losses)
